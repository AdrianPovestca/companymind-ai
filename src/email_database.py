...
# Set DB_PATH to a Render persistent-disk path in production, for example:
# DB_PATH=/var/data/emails.db
DB_PATH = Path(__import__('os').environ.get("DB_PATH", str(BASE_DIR / "emails.db")))
...
def _ensure_schema():
    ...
    for name, definition in {
        "urgent_notified_at": "TEXT", "decision_action": "TEXT", "decision_reason": "TEXT",
...
def save_email(email: Email, status: str = "new"):
    ...
    conn.execute("""INSERT OR IGNORE INTO emails(message_id,thread_id,sender,recipient,subject,body,timestamp,attachments,status,message_type)
        VALUES(?,?,?,?,?,?,?,?,?,?)""", (email.message_id, email.thread_id, email.sender, email.recipient,
        email.subject, email.body, timestamp, ",".join(map(str, email.attachments)), status, "email"))
    conn.commit(); conn.close()
