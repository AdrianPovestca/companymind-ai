...
GMAIL_CREDENTIALS_PATH = Path(
    os.environ.get(
        ...
        str(BASE_DIR / "gmail_oauth_credentials.json"),
    )
).expanduser()
    ...
    try:
        with _agent_lock:
            process = subprocess.Popen(
                ...
                env={
                    **os.environ,
                    "GMAIL_CREDENTIALS_PATH": str(GMAIL_CREDENTIALS_PATH),
        ...
        while not _agent_stop.is_set():
            for command in commands:
                ...
                if result not in (0, None) and not _agent_stop.is_set():
                    _agent_state["last_error"] = (
                        f"Step {' '.join(command[1:])} exited with code {result}. "
                        "Check Gmail authentication and token files."
                    )
...
def start_agent():
    if not GMAIL_CREDENTIALS_PATH.is_file():
        return jsonify({
            "ok": False,
            "error": f"Missing credentials file: {GMAIL_CREDENTIALS_PATH}",
            **_agent_state,
        }), 400
    ...
    if not GMAIL_TOKEN_PATH.is_file():
        return jsonify({
            "ok": False,
            "error": f"Missing token file: {GMAIL_TOKEN_PATH}",
            **_agent_state,
        }), 400
...
def get_email(email_id):
    try:
        ...
        conn = get_connection()
        row = conn.execute(
            "SELECT message_id, subject, sender, body, created_at FROM emails WHERE message_id = ?",
            (email_id,),
        ).fetchone()
        conn.close()
        if not row:
...
def approve(email_id):
    try:
        conn = get_connection()
        conn.execute(
            "UPDATE emails SET decision_action = 'approved', human_review_status = 'approved' WHERE message_id = ?",
            (email_id,),
        )
        conn.commit()
        conn.close()
...
def reject(email_id):
    try:
        conn = get_connection()
        conn.execute(
            "UPDATE emails SET decision_action = 'escalate', human_review_status = 'rejected' WHERE message_id = ?",
            (email_id,),
        )
        conn.commit()
        conn.close()
